...
- **SpatialQC** — the FastQC parallel for spatial omics: a *fast, run-first* QC mode (`POST /api/qc`) that runs only load → profile → tissue detection (no clustering) and returns a platform-aware PASS/WARN/FAIL report in seconds. Modules: Basic Statistics, Platform Detection Confidence, Library Size, Gene Detection Distribution, Sparsity/Dropout, Mitochondrial Content, Spatial Signal Sanity (Moran's I), Tissue Coverage, Image/Segmentation Quality — plus two spatial-specific analyses inspired by VoyagerPy / Space Ranger: **Spatial QC Maps** (total counts, genes detected, mitochondrial % rendered in tissue space) and **Spatially Variable Genes** (genes ranked by Moran's I, the `spatial_enrichment.csv` analog, with tissue-space expression maps). Delivered as a self-contained HTML report (figures + written interpretations + tables) plus a MultiQC-aggregatable `qc_summary.json`. Answer "is this dataset worth clustering?" before committing to the heavy benchmark.
...
- **One-click demos**: `GET /api/qc/demos` + `POST /api/qc/demo` run SpatialQC on bundled samples with no upload — a real 10x Visium breast tumor (GSM6433585 / Bassiouni et al.) and synthetic pass/fail examples. The web UI has a "Try a demo" control.
