"""
spatialMNN: spatial mutual nearest neighbors clustering.
...
def run(adata, n_clusters, seed=42, dataset="DLPFC", slide_id="unknown", **kwargs):
    """Run spatialMNN via R subprocess."""
    ...
    result = subprocess.run(
        [R_SCRIPT, r_script, h5ad_path, str(n_clusters), dataset, slide_id, str(seed), "spatialMNN"],
        capture_output=True, text=True, timeout=3600
    )

    if result.returncode != 0:
        raise RuntimeError(f"spatialMNN R script failed: {result.stderr[-500:]}")

    output_line = [l for l in result.stdout.strip().split("\n") if l.startswith("RESULT:")]
    if not output_line:
        raise RuntimeError(f"No RESULT line in spatialMNN output: {result.stdout[-500:]}")
